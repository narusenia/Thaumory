...
final class AttractionInfusion implements InfusionEffect {
    ...
    public void tick(InfusionContext context) {
        ...
        double radius = context.setting("radius", 3) + context.infusionLevel();
        for (ItemEntity item : context.level().getEntitiesOfClass(ItemEntity.class, context.wearer().getBoundingBox().inflate(radius),
